    ...
    <div className="rounded-lg border p-4 space-y-3">
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs px-2 py-0.5 rounded"
              style={{ background: HEALTH_COLOR[m.health] + "22", color: HEALTH_COLOR[m.health] }}>
          {m.health}
        </span>
      </div>
      <div className="text-sm">{m.status === "up" ? "Online" : "Offline"}</div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div>
          <div className="text-xs text-gray-500">CPU</div>
          <div className="font-mono">{m.cpu_percent}%</div>
          <Sparkline id={`${n.instance}:cpu`} value={m.cpu_percent} color="#2563eb" />
        </div>
        <div>
          <div className="text-xs text-gray-500">Memory</div>
          <div className="font-mono">{m.memory_percent}%</div>
          <Sparkline id={`${n.instance}:mem`} value={m.memory_percent} color="#7c3aed" />
        </div>
        <div>
          <div className="text-xs text-gray-500">Disk</div>
          <div className="font-mono">{m.disk_percent}%</div>
          <Sparkline id={`${n.instance}:disk`} value={m.disk_percent} color="#ea580c" />
        </div>
      </div>
